...
void DisplayAST(struct SymbolTableTree* node){

    if(node !=NULL){
        if(node -> nodeType == "OpenParan" || node -> nodeType == "CloseParan" || node -> nodeType == "OpenScope" || node -> nodeType == "TypeName" || node -> nodeType == "Empty" || node -> nodeType == "Comma" || node -> nodeType == "CloseScope" || node -> nodeType == "Colon" || node -> nodeType == "SemiColon" ) return;
        int nu = mm[node->nodeType]++;
        cout<<(node->nodeType+"("+to_string(nu)+")")<<"\n";
        if(node->nodeType == "Identifier"){
            cout<<("Identifier("+to_string(nu)+")`")<<(node->name + "(" + to_string(mm[node->name]++)+")")<<"\n";
            ...
            if (node->Nchildren > 0){
                ...
                for (i=0;i<node->Nchildren;i++){
                    if(node->child[i] -> nodeType == "OpenParan" || node->child[i]-> nodeType == "CloseParan" || node->child[i] -> nodeType == "OpenScope" || node->child[i] -> nodeType == "Empty" || node->child[i] -> nodeType == "TypeName" || node->child[i] -> nodeType == "Comma" || node->child[i] -> nodeType == "CloseScope" || node->child[i] -> nodeType == "Colon" || node->child[i] -> nodeType == "SemiColon" ) continue;
                    //std::cerr<<(node->child[i] -> nodeType)<<"\n";
                    cout<<(node->nodeType+"("+to_string(nu)+")`");
                    DisplayAST(node->child[i]);
                    // if(i != node->Nchildren - 1){
                    //     cout<<",";
                    // }
                }
...
CPro:						Main {check($1);typeCheck($1);DisplayAST($1);get_ast($1);$$=$1;$$ -> parent = NULL;check_icg($1);}
                        |   {yyerror("main function missing");}


Main:						T_TINT T_MAIN OpenParan CloseParan OpenScope Scope CloseScope {$$ = createEntry(yylineno, "Main", none, none, "int", 1, $6);error_taker = $$;}
...
Unary:						Id UnaryOp {$$ = createEntry(yylineno, "Unary", none, none, none, 2, $1, $2);error_taker = $$;}
                        ...
                        |	T_UNARY_OPERATOR2 Operand {$$ = createEntry(yylineno, "Unary", none, none, none, 2, makeOpNode("UnaryOp",$1, yylineno), $2);error_taker = $$;}
...
void check_icg(struct SymbolTableTree*node){
    ...
    ass_creator(icg, mapper);
}

void printST(){
    cerr<<"\n\n\n"<<("name")<<":\t"<<("nodeType")<<"\t"<<("dataType")<<"\t"<<("value")<<"\n";
    for(auto t : mapper){
        if(t.second.dataType.length()<8)
